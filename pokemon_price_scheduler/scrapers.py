...
from .models import PriceObservation, Source, SourceResult
from .parsing import (
    clean_text,
...
def snkrdunk_search_url(keyword: str, page: int = 1) -> str:
    """Build a SNKRDUNK API search URL from a keyword."""
    params = (
    ...
    def scrape(self, source: Source) -> SourceResult:
        try:
            if source.kind == "snkrdunk_api" or source.kind == "snkrdunk_search":
                # Fetch from SNKRDUNK's JSON API instead of the page URL
...
def extract_snkrdunk_api(source: Source, raw_text: str) -> list[PriceObservation]:
    """Parse SNKRDUNK JSON API response (not HTML)."""
    items: list[PriceObservation] = []
    seen: set[tuple[str, int]] = set()
    ...
    def walk(data):
        if isinstance(data, dict):
            ...
            if isinstance(products, list) and products:
                ...
                return  # Found products, no need to recurse deeper
