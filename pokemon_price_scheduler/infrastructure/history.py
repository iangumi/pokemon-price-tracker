...
def get_all_products_with_trend() -> list[dict]:
    ...
    try:
        rows = conn.execute(
            """
            WITH latest AS (
                SELECT DISTINCT slug, title, language, own_price_idr, global_average_idr, price_delta_percent, alert_level, alert_label, run_id
                FROM product_results
                WHERE id IN (
            ...
            previous AS (
                SELECT DISTINCT slug, own_price_idr, global_average_idr
                FROM product_results pr1
                WHERE id = (
                    SELECT MAX(id) FROM product_results
                    WHERE slug = pr1.slug AND id < (
                        SELECT MAX(id) FROM product_results WHERE slug = pr1.slug
                    )
                )
            )
            SELECT
                l.slug, l.title, l.language, l.own_price_idr,
                l.global_average_idr, l.price_delta_percent, l.alert_level, l.alert_label,
                p.own_price_idr AS prev_own_price_idr,
                p.global_average_idr AS prev_global_average_idr
            FROM latest l
            LEFT JOIN previous p ON l.slug = p.slug
            ORDER BY l.title
            """,
        ).fetchall()
        products = []
